...
function Page() {
  ...
  return <li key={i} className='flex items-center justify-between bg-pink-300 mb-5 rounded text-nowrap'>
  <div className=' flex justify-around mb-5 w-2/3 pt-3' >
    <h5 className='mr-4 ml-4'>{t.title}</h5>
      <h5 className='mr-4'>{t.desc}</h5>
  </div>
  ...
  return (
    <>
      <h1 className='bg-black text-white p-5 text-2xl text-center w-full'>ToDo List</h1>
      <form onSubmit={handleSubmit} className='flex justify-center mt-10 mb-5 flex-wrap'>
        <input
          type='text'
